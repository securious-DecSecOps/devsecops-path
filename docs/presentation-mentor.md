...
footer: 'dsaedsae · github.com/securious-DecSecOps · v0.3.2-poc'
style: |
...
Build #21 시점에 5종 정적 도구(Trivy/Checkov/Gitleaks/SonarQube/Kubescape) + SBOM이 통합된 16-stage 파이프라인이 사람 개입 없이 한 사이클 끝까지 닫혔다.
...
Build #21 시점에도 verify.sh가 7/7 PASS를 보고하면서 4가지 모두 재현된다.
...
9.  Kubescape K8s Manifest Scan     — NSA-CISA + MITRE + CIS framework
10. Checkov IaC Scan                — Dockerfile + Helm chart
11. Gitleaks Secret Scan            — 3 repo git history
12. SonarQube SAST                  — PHP
13. Security Gate                   — 5 도구 결과 aggregate (ENFORCE_GATE 정책)
14. Registry Login + Push           — Harbor
15. Deploy + GitOps Commit Push     — gitops repo에 image tag 자동 commit/push
16. Collect + Archive Evidence
```

Build #21에서 16 stage 전부 통과. 트리거 한 번이면 빌드부터 배포까지 사람 손이 안 들어간다.
...
Build #21 결과로는 각 서비스당 SPDX 170 packages / CycloneDX 169 components. 같은 base image(`php:7.4-cli`)를 쓰니까 패키지 수가 거의 동일한 게 자연스럽다.

운영 단계 가면 이걸 그대로 Dependency-Track에 POST하면 신규 CVE 매칭이 자동으로 돌아간다. PoC는 파일 생성까지만.

---

## Kubescape — K8s-specialized 정적 스캔 + 프레임워크 매핑

Checkov가 multi-tech(Terraform/Docker/K8s) IaC 도구라면 Kubescape는 K8s 전용이다. 가장 큰 차별점은 정부/규제 프레임워크에 직접 매핑된 결과를 낸다는 점.

`scripts/kubescape-scan.sh`가 Helm chart(`gitops-manifest-repo/helm/vulnbank-msa`)를 대상으로 3개 framework로 각각 스캔:

- **NSA-CISA Kubernetes Hardening Guide**
- **MITRE ATT&CK for Kubernetes**
- **CIS Kubernetes Benchmark** (v1.12.0)

Build #21 결과 (실제 vulnbank-msa Helm chart, 16 resources 평가):

- NSA: 14/20 controls 통과 → 6 failed
- MITRE: 16/17 controls 통과 → 1 failed
- CIS v1.12.0: 26/33 controls 통과 → 2 failed

각 framework의 raw JSON + summary가 `reports/dev/21/kubescape/`에 archive. 운영 시 ENFORCE_GATE=true로 토글하면 Security Gate가 Kubescape 결과도 aggregate해서 빌드를 차단할 수 있다. PoC는 정보 기록까지.

다음 단계로는 in-cluster operator 모드(`gitops/platform/kubescape/`)에서 runtime posture 모니터링 자리가 확정되어 있다.

---

## SonarQube가 찾은 PHP 측 약점

SonarQube 기준 Build #21:
...
## GitOps 루프가 닫히는 모습 (Build #21 evidence)

```
Developer가 app-source-repo에 push
   ↓
Jenkins Build #21 (16 stage)
   ↓
Harbor에 :21 태그로 6개 이미지 push
   ↓
Jenkins가 gitops-manifest-repo에 image tag bump를 자동 commit/push
   ↳ e5e830e ci: bump vulnbank-msa to image tag 21 (build #21)
   ↓
ArgoCD detect + sync (필요 시 hard refresh annotation)
   ↓
새 frontend Pod 기동 with image :21
   ↓
verify.sh 7/7 PASS
```
...
Build #21 직전에 발견한 갭. backend는 Phase 4에서 HMAC Bearer token으로 전환했는데, 브라우저에서 도는 `vulnbank.js`는 cookie session 그대로였다. 그래서 브라우저로 로그인까지는 되지만 그 다음 ajax 호출은 전부 "Permission denied"로 떨어지고 있었다.
...
`archiveArtifacts`로 Jenkins archive에 영구 보존. Build #21로 들어가면 위 디렉토리 트리 그대로 조회 가능.
...
13. Kubescape의 옛 `kubescape-ubuntu-latest` 자산 명명이 deprecated(404) → `kubescape_<ver>_linux_amd64.tar.gz` 패턴 + `kubescape list frameworks`로 정확한 CIS framework 이름 확인 후 pin
...
docker exec secubank-jenkins ls \
  /var/jenkins_home/jobs/vulnbank-msa-dev/builds/21/archive/reports/dev/21/sbom/
...
- Build #21 Jenkins console 캡처
- SonarQube dashboard 캡처 (8 vulns / 117 bugs / 42 hotspots)
- Harbor secubank/vulnbank-msa-* 태그 리스트 (`:21` 보이게)
- ArgoCD Synced + Healthy 캡처 (`rev=e5e830e`)
...
- 3 repo 모두 public + tag `v0.3.2-poc` 박힌 상태 확인
